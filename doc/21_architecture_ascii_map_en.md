# 21 Architecture ASCII Map

This file preserves the useful “single-screen mental model” from the old monolingual ASCII map, but folds it into the maintained bilingual suite.
         ...
         +--> lmms_eval.evaluator.simple_evaluate(...)
                ...
                +--> algorithm/focus/interface.py::apply_focus(...)
                       ...
                       +--> patched decoder / attention / MLP forwards
                              |
                              +--> Focus.forward(...)              [SIC path]
                              +--> Focus.set_token_importance(...)
                              +--> Focus.semantic_concentration(...) [SEC path]
                              +--> Focus.post_process(...)
                                       |
                                       +--> focus_main/*.pth
                                       +--> focus_int8/*.pth
...
simulator/main.py
  ...
  +--> simulator/models/sparse_info.py::SparseInfo(trace or sparsity CSV)
  +--> simulator/arch/accelerator.py::Accelerator(hardware config)
...
```text
algorithm output CSVs     simulator output CSVs     evaluation_scripts/jetson_stats/figure9_gpu.csv
          |                         |                                   |
          +-------------------------+-----------------------------------+
                                    |
                                    v
...
- Reasoned inference: the repository methodology is “software traces drive architectural simulation; synthesized RTL calibrates area/power,” not “one integrated hardware/software co-simulation stack.”
